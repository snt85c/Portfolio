...
export default function Devicons(props: { icons: iDevicons }) {
  ...
  const SIZE: number = width > 600 ? width / 45 : width / 15;

  let dependencies = props.icons.dependencies.map((dependency, i) => {
    return (
      <span
        className="text-white px-2 bg-slate-800/75 rounded-full text-[0.6rem] sm:text-[1vw] ml-1"
        key={i}
      >
        #{dependency}
      </span>
  ...
  const svgPaths: {
    ...
    {
      path: <Icon icon="logos:framer" width={SIZE} height={SIZE} />,
      name: "Framer-Motion",
      src: "https://www.framer.com/motion/",
      type: "iconify",
    },
    {
      path: (
    ...
    {
      path: <Icon icon="logos:testing-library" width={SIZE} height={SIZE} />,
    ...
    {
      path: <Icon icon="logos:vercel"  width={SIZE} height={SIZE} />,
      name: "Vercel",
      src: "https://vercel.com/",
      type: "iconify",
    },
    {
      path: <Icon icon="logos:cypress-icon" width={SIZE} height={SIZE} />,
      name: "Cypress",
      src: "https://www.cypress.io/",
      type: "iconify",
    },
    {
      path: (
  ...
  const deviconsFactory: any = svgPaths
    .filter((item) => props.icons.icons.indexOf(item.name) !== -1)
    .map((item, i) => {
      switch (item.type) {
        case "devicons":
          return (
            <a href={item.src} target={"_blank"} rel="noreferrer">
              ...
              >
                <svg
                  ...
                  width={SIZE}
                  height={SIZE}
                  className="hover:-translate-y-2 duration-300 hover:opacity-100 "
                >
                  {item.path}
                </svg>
                <div className=" absolute sm:-bottom-3 -bottom-5 text-black text-[0.7rem] opacity-0 group-hover:opacity-100 duration-300 ">
                  {item.name}
                </div>
              </div>
            </a>
          ...
          return (
            <a href={item.src} target={"_blank"} rel="noreferrer">
              <div
                ...
                key={i}
              >
                <div className="hover:-translate-y-2 duration-300 hover:opacity-100 ">
                  {item.path}
                </div>
                <div className=" absolute flex justify-center items-center text-center w-[200px] sm:-bottom-3 -bottom-5 text-black text-[0.7rem] opacity-0 group-hover:opacity-100 duration-300 ">
                  {item.name}
                </div>
              </div>
            </a>
